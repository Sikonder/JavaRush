...
public class FunctionalTest {
    public void testStorage(Shortener shortener){
        ...
        Assert.assertEquals(threeID,oneID);

        String first = shortener.getString(oneID);
        String second = shortener.getString(twoID);
        String third = shortener.getString(threeID);

        Assert.assertEquals(first,one);
        Assert.assertEquals(second,two);
        Assert.assertEquals(third,three);
    }
    @Test
